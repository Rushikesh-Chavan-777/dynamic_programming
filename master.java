...
public class master {
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //now, solving the first 3d dp proble,, which is chocolate puckup by two starting positions 
        ...
        for(int j = -1; j <=1; j++) {
            for(int k = -1; k <=1; k++) {
                if(j1 == j2) {
                    ...
                    maxi = Math.max(maxi, matrix[i][j1] + matrix[i][j2] + chocolatepickupUtil(i+1,j1 + j, j2 + k, n, m, matrix, dp));
                }
            }
        }
        ...
        for(int i = n-1; i >=0; i++) {
            for(int j1 = -1; j1 <= +1; j1++) {
                for(int j2 = -1; j2 <= +1 ; j2++) {
                    if(j1 == j2) {
                    maxi = Math.max(maxi, matrix[i][j1] + dp[i-1][j1][j1]);
                }else {
                    maxi = Math.max(maxi, matrix[i][j1] + matrix[i][j2] + dp[i-1][j1][j2]);
                }
                dp[i][j1][j2] = maxi;
                }  
            }
